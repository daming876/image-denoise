...
class DatasetUSRNet(data.Dataset):
    ...
    # Only "paths_H" and kernel is needed, synthesize L on-the-fly.（动态合成L）
    # -----------------------------------------
    '''
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            #print("kernelshape--k:", k.shape)  #（25，25）,接着查找如何变为torch.size([48,1,25,25])
            #print("kernel值--k:",k)    #[[X,X,X....
            ...
            k /= np.sum(k)
            print("dataset_usrnet_validation-k.shape",k.shape)
        ...
        #print("dataset_axis=2-k.shape:",k.shape)    #dataset_axis=2-k.shape: torch.Size([1, 25, 25])
        img_H, img_L = util.uint2tensor3(img_H), util.single2tensor3(img_L)
        noise_level = torch.FloatTensor([noise_level]).view([1,1,1])
        #print("l:{},k:{},sf:{},sigma:{}".format(img_L,k,self.sf,noise_level))
        #查看数据结构sigma:torch.Size([1, 1, 1])
        # print("l:{},k:{},sf:{},sigma:{}".format(img_L.shape,k,self.sf,noise_level.shape))
