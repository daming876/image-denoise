...
def p2o(psf, shape):
    ...
    Returns:
        otf: NxCxHxWx2
    '''
    #print("shape:",shape)    #shape:(132,132)
    #psf=torch.tensor(psf)
    print("psfshape:", psf.shape)
    #print("psf:", psf)

    otf = torch.zeros(psf.shape[:-2] + shape).type_as(psf)
    #otf = torch.zeros(psf.shape[:-2] + shape).type_as(torch.tensor(psf))

    print("otfshape:", otf.shape)
    #print("otf:", otf)

    otf[...,:psf.shape[2],:psf.shape[3]].copy_(psf)
    #otf[..., :psf.shape[2], :psf.shape[3]].copy_(torch.tensor(psf))
...
"""
# --------------------------------------------
# (1) Prior module; ResUNet: act as a non-blind denoiser，先验模块，被看作是去噪的过程， 
# x_k = P(z_k, beta_k)，因为噪声可以用先验知识预设，因此被称为先验项。这里采用基于学习（深度学习）的方法来进行求解得到去噪后的图像x
# --------------------------------------------
"""
...
"""
# --------------------------------------------
# (2) Data module, closed-form solution数据模块，实现基于模型的方法对图像进行超分辨的重建
# It is a trainable-parameter-free module  ^_^这种基于模型的方法可以对任意scale,任意模糊核进行超分辨重建
...
"""
# --------------------------------------------
# (3) Hyper-parameter module，计算每层迭代需要的超参数
# --------------------------------------------
"""
...
class USRNet(nn.Module):
    ...
    def forward(self, x, k, sf, sigma):
        ...
        w, h = x.shape[-2:]      #对x的shape进行切片操作，取倒数第二和第一个值,即把w,h取出
        FB = p2o(k, (w*sf, h*sf))  #w,h都乘以sf(超分倍数)
        FBC = torch.conj(FB)    #torch.conj（）创建共轭张量。input为(Tensor) -输入张量，返回带有翻转共轭位的input的视图。
        F2B = torch.pow(torch.abs(FB), 2)    #实现张量和标量之间逐元素求指数操作,或者在可广播的张量之间逐元素求指数操作.
        STy = upsample(x, sf=sf)
        FBFy = FBC*torch.fft.fftn(STy, dim=(-2,-1))
        x = nn.functional.interpolate(x, scale_factor=sf, mode='nearest')

        #print("XXXXXXXXXXsigma:",len(sigma))   #sigma=batchsize,48
        sigma=sigma.reshape(len(sigma),1,1,1)   #to dim=4,新加
        print("XXXXXXXXXXsigma:", sigma.shape)  #torch.Size([48, 1, 1, 1])
        print("XXXXXXXXXXsf:", sf)     #sf=3

        # hyper-parameter, alpha & beta，torch.cat函数把sigma和sf(超分的倍数)连接为1维的数据
