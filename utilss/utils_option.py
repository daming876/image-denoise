...
def get_timestamp():
    return datetime.now().strftime('_%y%m%d_%H%M%S')   #strftime(format[, tuple]) -> string根据指定的格式化字符串输出

#参数opt_path为不同网络对应json的路径
def parse(opt_path, is_train=True):

    # ----------------------------------------
    # remove comments starting with '//'   删除以“//”开头的注释
    # ----------------------------------------
    json_str = ''
    ...
    print('XXXXXXXXXXXXXXXXXXXX export CUDA_VISIBLE_DEVICES=' + gpu_list)
...
def save(opt):
    ...
    dirname, filename_ext = os.path.split(opt_path)    #将path分割成目录和文件名二元组返回
    filename, ext = os.path.splitext(filename_ext)    #分离文件名与扩展名；
